...
class FaceDetectorV2:
    ...
    def get_passport_crop(self, image, side_coeff=2.3):
        results = self.detector(image, verbose=False)[0]
        if not results.boxes: return None, None
        ...
        cy_adj = cy + int(side * 0.1) # Сдвиг для макушки
        ...
        # Возвращаем кроп и центр лица относительно кропа для сегментатора
